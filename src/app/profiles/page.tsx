...
// fetcher no-store pour bypass tout cache
const fetcher = (url: string) =>
  fetch(url, { cache: "no-store" })
    .then((r) => r.json())
    .then((j) => j.profiles as ProfileDto[]);

export default function ProfilesPage() {
  const [selected, setSelected] = useState<any>(null);

  // SWR : auto-refresh + revalidate on focus
  const {
    data: items = [],
    isLoading,
    mutate,
    ...
    dedupingInterval: 5000,     // évite de spammer si plusieurs appels
    revalidateOnFocus: true,    // quand on revient sur l’onglet
  ...
  const grouped = useMemo(() => {
    const g: Record<UiRole, ProfileDto[]> = {
      TOP: [], JUNGLE: [], MID: [], ADC: [], SUPPORT: [], SUB: [],
    };
  ...
  return (
    <main className="min-h-screen">
                    ...
                    {grouped[role].map((p) => {
                      ...
                      return (
                        <ProfileBubble
                          ...
                          points={p.points ?? 0}
                          onClick={() =>
                            setSelected({ ...p, name, avatar: p.avatar })
                          }
                        />
                      );
                    })}
      ...
      <ProfileModal
        ...
        onSaved={async () => {
          setSelected(null);
          await mutate(); // re-fetch immédiat après sauvegarde
        }}
      />
    </main>
  );
